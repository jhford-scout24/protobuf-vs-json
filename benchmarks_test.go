...
func TestSerialisedSizes(t *testing.T) {
	...
	for i := 0; i <= 1024; i = 1 + i*2 {
		...
		tests[fmt.Sprintf("%d_fields", i)] = tc
	}
	...
	for i := 0; i <= 1024; i = 1 + i*2 {
		...
		tests[fmt.Sprintf("%d_metrics", i)] = tc
	}
	...
	for test, details := range tests {
		...
		t.Run(fmt.Sprintf("%s PROTOBUF: %d bytes", test, pSize), func(t *testing.T) {})
		t.Run(fmt.Sprintf("%s JSON:     %d bytes", test, jSize), func(t *testing.T) {})
	}
}
...
func BenchmarkIt(b *testing.B) {
	...
	for test, details := range tests {
		...
		b.Run(fmt.Sprintf("%s PROTOBUF SERIALISE", test), func(b *testing.B) {
			var size int
			for i := 1; i < b.N; i++ {
		...
		b.Run(fmt.Sprintf("%s JSON SERIALISE", test), func(b *testing.B) {
			var size int
			for i := 1; i < b.N; i++ {
		...
		b.Run(fmt.Sprintf("%s PROTOBUF PARSE", test), func(b *testing.B) {
			b.StopTimer()
		...
		b.Run(fmt.Sprintf("%s JSON PARSE", test), func(b *testing.B) {
			b.StopTimer()
		...
		b.Run(fmt.Sprintf("%s JSON VALIDATE-PARSE", test), func(b *testing.B) {
			b.StopTimer()
			buf := tester.SerialiseJSON(j)
			b.ResetTimer()
			b.StartTimer()
